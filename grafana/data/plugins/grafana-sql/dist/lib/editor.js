...
System.register([], function (_export, _context) {
  ...
  var _typeof, CodeMirror, SqlFormater;
  ...
  return {
    ...
    execute: function () {
      ...
      _export("CodeMirror", CodeMirror = function () {
      ...
      _export("SqlFormater", SqlFormater = {
        defualt: {
          keywords: 'select|from|where|join|on|group by|having|order by|limit|left join|right join|inner join|outer join|' + 'delete from|insert into|values|update|set|' + 'create table|drop table|alert table|add',
          uppercase: 'as|asc|desc|like|is|null|not|in|and|or',
          hock: {
            // 默认格式化,逗号分隔
            'default_format': function default_format(str, offset) {
              var pre = new Array(offset).join(' ');
              return str.replace(/\(.*?\)/ig, function (match) {
                return match.replace(/,/ig, '_#');
              }).replace(/,/ig, ',\n' + pre).replace(/\_#/ig, ',');
            },
            'select': function select(str, offset) {
              return this.default_format(str, offset);
            },
            'set': function set(str, offset) {
              return this.default_format(str, offset);
            },
            'add': function add(str, offset) {
              return this.default_format(str, offset);
            },
            'create table': function createTable(str, offset) {
              var pre = new Array(offset + str.indexOf('(')).join(' ');
              return str.replace(/,/ig, ',\n' + pre);
            },
            'where': function where(str, offset) {
              var keywords = 'and|or';
              var off_info = SqlFormater.maxoffset(str, keywords);
              ...
              var subs = SqlFormater.child(str, '(');
              str = subs.target;
        ...
        format: function format(sql, options, offset) {
          ...
          // format inline
          var sql = sql.replace(/^\s+|\s+$|;/ig, '').replace(/\n/ig, '').replace(/\s+/ig, ' ').replace(/\(\s*(select)/ig, '($1');
          ...
          // format
          var i = 0;
          sql = sql.replace(new RegExp(keywords, 'ig'), '}$&{').substring(1).concat('}').replace(new RegExp('(' + keywords + '){(.*?)}', 'ig'), function (match, func, args) {
        ...
        maxoffset: function maxoffset(str, keywords) {
          var max_offset = 0;
          var i = 0;
        ...
        child: function child(str, search) {
          var res = [];
